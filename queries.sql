...
SELECT species,
  AVG(escape_attempts)
FROM animals
WHERE date_of_birth >= '01-01-1990'
  AND date_of_birth < '12-31-2000'
GROUP BY species;
SELECT animals.name
FROM animals
...
WHERE vet.name = 'Stephanie Mendez'
  AND v.date_of_visit BETWEEN 'Apr 1, 2020' AND 'Aug 30, 2020';
--most visits 
SELECT an.name,
  COUNT(an.name) AS most_visits
FROM visits v
  JOIN animals an ON an.id = v.animal_id
GROUP BY an.name
...
SELECT vet.name AS vet,
  COUNT(*)
FROM visits vis
  JOIN vets vet ON vet.id = vis.vet_id
  LEFT JOIN specializations s ON s.vet_id = vis.vet_id
WHERE s.id IS NULL
GROUP BY vet.name;
